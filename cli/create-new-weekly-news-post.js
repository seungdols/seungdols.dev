...
const getNumberOfLastWeeklyNewsPost = () => {
  return fs
    .readdirSync(TARGET_DIR, (err, items) => {
      if (err) {
        return console.log(`Unable to scan directory:  ${err}`)
      }
      return items
    })
    .filter(el => el.includes('weekly-news'))
    .map(el =>
      el
        .split('.')
        .slice(0, -1)
    ...
    .map(el => el.split('-').pop())
    .reduce((previos, current) => (previos > current ? previos : current))
}
...
const createWeeklyNewsPost = lastNumber => {
  ...
  const fileName = `${dateFns.format(
    new Date(),
    DATE_FORMAT
  )}-weekly-news-${newNumberWeeklyNews}`

  fs.writeFile(
    `${TARGET_DIR}/${fileName}.markdown`,
    getTemplate(newNumberWeeklyNews),
    err => {
      if (err) {
